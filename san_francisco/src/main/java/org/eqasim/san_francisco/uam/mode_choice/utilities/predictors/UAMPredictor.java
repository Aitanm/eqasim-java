package org.eqasim.san_francisco.uam.mode_choice.utilities.predictors;
...
public class UAMPredictor {
	private static final Logger log = Logger.getLogger(UAMPredictor.class);
	...
	@Inject
	public UAMPredictor(UAMManager manager, Scenario scenario, WaitingStationData waitingData, UAMConfigGroup uamConfig,
			TransitConfigGroup transitConfigGroup, Network carNetwork,
			LeastCostPathCalculatorFactory lcpcf, TripRouter transitRouter, CustomModeChoiceParameters parameters,
			UAMStationConnectionGraph stationConnectionutilities, SubscriptionFinder subscriptions, Map<String,TravelDisutilityFactory> travelDisutilityFactories,Map<String, TravelTime>
			  travelTimes) {
		this.manager = manager;
		this.scenario = scenario;
		this.waitingData = waitingData;
		this.uamConfig = uamConfig;
		this.availableModes = uamConfig.getAvailableAccessModes();;
		...
		this.plcpccar = lcpcf.createPathCalculator(carNetwork, travelDisutility, travelTime);
	}

	public double calcMaxUtility(Person person, DiscreteModeChoiceTrip trip) {

		Link originLink = this.carNetwork.getLinks().get(trip.getOriginActivity().getLinkId());
		Link destinationLink = this.carNetwork.getLinks().get(trip.getDestinationActivity().getLinkId());

		UAMTripData UAMTrip = calcBestRoute(person, originLink, destinationLink, trip.getOriginActivity().getEndTime());

		return UAMTrip.maxUtility;
	}
...
//		}
		...
		// What is the usage of the following code?? here and in the original version
		// TODO

		/*
		 * try { UAMRoutes.getInstance().add(person.getId(), departureTime, new
		 * UAMRoute(accessModeMap.get(bestStationOrigin.getId()), bestStationOrigin,
		 * bestStationDestination, egressModeMap.get(bestStationDestination.getId())));
		 * } catch (NullPointerException e) { // do not store best UAM route if any part
		 * of the UAM trip cannot be estimated // (e.g. no destination station)
		 * log.warn("Requested UAM prediction for person " + person.getId() +
		 * " did not provide all UAM trip parts."); // return Double.NEGATIVE_INFINITY;
